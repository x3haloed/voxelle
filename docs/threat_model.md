...
**Existing controls.** Cargo locking, repository tests, CI builds on macOS and Windows, ad-hoc macOS signing, native Tauri packaging, SHA-256 manifests, and narrow per-app trust instructions provide reproducibility and corruption detection. The guide explicitly rejects global Gatekeeper disablement and does not claim vendor signing.

**Review focus.** A checksum stored beside an artifact on the same compromised provider is not independent authenticity. Release integrity needs a clearly authenticated root—reviewed source/tag, reproducible builds, signed provenance, threshold-maintained manifest, or another explicit mechanism—before resisting a malicious update provider can be claimed. CI permissions should remain minimal, release and ordinary PR workflows separated, dependencies audited, generated files verified, and debug/test secret backends excluded from release. An update-provider compromise has malware-level severity because the app handles root, recovery, private-room, media, and governance capabilities.
