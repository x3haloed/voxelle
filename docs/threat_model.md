...
**Existing controls.** The UI generally constructs DOM nodes and assigns untrusted values through `textContent`; Rust validates replicated bodies and attachment hashes/sizes. Tauri exposes one serialized command entrypoint backed by a mutex and typed Serde requests, and its declared capability file only enables native snapshot event listening. Runtime truth stays in Rust. The human surface requires explicit modal review before product-package installation, staged-generation activation, rollback, or release-root transition, including when the semantic command starts in the palette; this is user-presence friction, not update authentication.
